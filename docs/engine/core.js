...
=============================================================== */
const gamePath = window.location.pathname.includes('/text-game/')
  ? '/text-game/docs/games/demo.json'
  : './games/demo.json';:

fetch(gamePath)
  .then(response => {
    if (!response.ok) throw new Error("Scene file not found");
  ...
  .catch(err => {
    ...
    document.getElementById("text-box").textContent = "Error loading game.";
  });
