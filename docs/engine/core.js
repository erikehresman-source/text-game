...
function maybeTriggerEvent() {
  if (Math.random() < 1) { // 30% chance per scene
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    await loadEvents("events/events.json");
    console.log("✅ Events file loaded successfully from /events/");
  } catch (err) {
  ...
  // TEMPORARY TEST: Trigger a random event after startup
setTimeout(() => {
  if (window.eventsData && window.eventsData.events && window.eventsData.events.length > 0) {
    const randomIndex = Math.floor(Math.random() * window.eventsData.events.length);
    const eventData = window.eventsData.events[randomIndex];

    console.log("🎲 Triggering test event:", eventData.id);
    if (typeof displayEvent === "function") {
      displayEvent(eventData);
    } else {
      console.warn("⚠️ displayEvent() not found or not loaded.");
    }
  } else {
    console.warn("⚠️ No events loaded to trigger.");
  }
}, 3000);
