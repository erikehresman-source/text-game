...
Safe.log('[ENGINE:BOOT] Base set to:', BASE);
  const ENGINE_TAG = '[ENGINE:BOOT]';
// --- Ensure BASE includes repo root when hosted on GitHub Pages ---
if (!BASE.includes('/text-game/')) {
  BASE = BASE.replace('https://erikehresman-source.github.io/', 'https://erikehresman-source.github.io/text-game/');
  Safe.warn('[BOOT] Adjusted BASE for GitHub Pages:', BASE);
}
...
async function loadRegistry() {
  const REGISTRY_URL = BASE.endsWith('core/universal/')
  ? `${BASE}registry.json`
  : `${BASE}core/universal/registry.json`;
  Safe.log("[ENGINE:BOOT] Registry fetch URL:", REGISTRY_URL);


try {
  const res = await fetch(REGISTRY_URL, { cache: "no-store" });
  Safe.log("[ENGINE:BOOT] registry response status:", res.status, res.statusText);
if (!res.ok) {
  throw new Error(`[ENGINE:BOOT] Registry fetch failed (${res.status}) at ${REGISTRY_URL}`);
}
    const data = await res.json();
	Safe.log('[REGISTRY] Loaded', data);
if (!data || typeof data !== 'object') {
  Safe.warn('[REGISTRY] Unexpected structure', data);
}
...
// --- Resolve Module URLs Relative to BASE (auto-detect /docs/) ---
function resolveModuleUrl(modulePath) {
  try {
    const path = window.location.pathname || '';
    const prefix = path.includes('/docs/') ? 'docs/' : '';
    // if caller gives absolute URL or root path, respect it
if (/^https?:\/\//.test(modulePath) || modulePath.startsWith('/')) {
  return modulePath;
}

// build a safe URL using the URL API (doesn't break https://)
const combined = `${prefix}${modulePath}`;
const url = new URL(combined, BASE).toString();
    Safe.log('[RESOLVE]', { pathname: path, modulePath, resolved: url });
    return url;
  } catch (err) {
...
// diagnostic logging (moved *below* the map)
Safe.log("[BOOTSTRAP] Core path resolved to:", map.core);
...
window.__UniversalBootstrapped = true;
...
Safe.try(() => {
  ...
  test_core: resolveModuleUrl('core/universal/core.universal.js'),
  test_registry: resolveModuleUrl('core/universal/registry.json')
});
