...
// --- Global Safe Utility (must be defined before bootstrap) ---
window.Safe = {
  log: (...a) => console.log('[ENGINE:BOOT]', ...a),
  info: (...a) => console.info('[ENGINE:BOOT]', ...a),
  warn: (...a) => console.warn('[ENGINE:BOOT]', ...a),
  error: (...a) => console.error('[ENGINE:BOOT]', ...a),
  try(fn, label = 'block') {
    try { return fn(); }
    catch (e) {
      console.error(`[ENGINE:BOOT] ${label} failed:`, e);
      return undefined;
    }
  }
};
